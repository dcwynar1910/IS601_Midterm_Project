...
def test_default_configuration():
    ...
    assert config.return_log_dir.resolve() == Path('./test_logs').resolve()
    assert config.return_history_dir.resolve() == Path('./test_history').resolve()
    assert config.return_history_file.resolve() == Path('./test_history/test_history.csv').resolve()
    assert config.return_log_file.resolve() == Path('./test_logs/test_log.log').resolve()
...
def test_directory_properties():
    ...
    assert config.return_log_dir.resolve() == Path('/custom_base_dir/logs').resolve()
    assert config.return_history_dir.resolve() == Path('/custom_base_dir/history').resolve()

def test_file_properties():
    clear_env_vars('CALCULATOR_HISTORY_FILE', 'CALCULATOR_LOG_FILE')
    config = CalculatorConfig(base_dir=Path('/custom_base_dir'))
    assert config.return_history_file.resolve() == Path('/custom_base_dir/history/calculator_history.csv').resolve()
    assert config.return_log_file.resolve() == Path('/custom_base_dir/logs/calculator.log').resolve()
...
def test_get_project_root():
    ...
    #from app.calculator_config import get_path_proj
    config = CalculatorConfig()
    assert (Path(str(config.get_path_proj() / "app"))).exists()  
...
def test_log_dir_property():
    ...
    assert config.return_log_dir.resolve() == Path('/new_base_dir/logs').resolve()
...
def test_history_dir_property():
    ...
    config = CalculatorConfig(base_dir=Path('/new_base_dir'))
    assert config.return_history_dir.resolve() == Path('/new_base_dir/history').resolve()
...
def test_log_file_property():
    ...
    config = CalculatorConfig(base_dir=Path('/new_base_dir'))
    assert config.return_log_file.resolve() == Path('/new_base_dir/logs/calculator.log').resolve()
...
def test_history_file_property():
    ...
    config = CalculatorConfig(base_dir=Path('/new_base_dir'))
    assert config.return_history_file.resolve() == Path('/new_base_dir/history/calculator_history.csv').resolve()
